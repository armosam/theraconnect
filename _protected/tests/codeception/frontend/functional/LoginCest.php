...
class LoginCest
{
    ...
    /**
     * Test login process when wrong attempts not exiting limit in configuration.
    ...
    public function testLoginWhenAttemptsNotExitingLimit($I)
    {
        ...
        $I->expectTo('see that Verification Code is not on the page and failed attempts equal 3');
        $I->dontSee('Verification Code');
        $I->canSeeRecord(LoginAttempt::class, ['ip'=>'::1', 'failed_attempts' => 3]);
    }

    /**
     * Test login process when wrong attempts exiting limit in configuration.
     * It will try to login with username and email as well
     *
     * @param $I FunctionalTester
     */
    ...
    /**
     * We want to be sure that not active user can not login with username.
    ...
    /**
     * We want to be sure that not active user can not login with email.
     * If he try to login, he should get error flash message and activation email again.
     *
     * @param $I FunctionalTester
     */
