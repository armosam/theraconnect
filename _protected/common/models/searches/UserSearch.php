...
class UserSearch extends User
{
    ...
    {
        return [
            [['username', 'email', 'title', 'full_name', 'first_name', 'last_name', 'status', 'item_name', 'service_id', 'credential_status'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        if (!Yii::$app->user->can('theCreator')) {
            $query = User::find()->joinWith('role')->where(['!=', 'auth_assignment.item_name', User::USER_SUPER_ADMIN]);
        } else {
        ...
        $query->andFilterWhere(['ilike', '[[user.username]]', $this->username])
              ...
              ->andFilterWhere(['or',
                  ['ilike', 'CONCAT_WS(\' \', [[user.first_name]], [[user.last_name]])', $this->full_name],
                  ['ilike', '[[user.agency_name]]', $this->full_name]
              ])
